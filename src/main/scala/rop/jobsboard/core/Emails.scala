...
trait Emails[F[_]] {
  def sendEmail(from: String, to: String, subject: String, content: String): F[Unit]
  def sendPasswordRecoveryEmail(from: String, to: String, token: String): F[Unit]
}
...
class LiveEmails[F[_]: MonadCancelThrow] private (emailServiceConfig: EmailServiceConfig) extends Emails[F] {
  ...
  val username: String    = emailServiceConfig.username
  val password: String    = emailServiceConfig.password
  ...
  override def sendEmail(from: String, to: String, subject: String, content: String): F[Unit] = {
  ...
  override def sendPasswordRecoveryEmail(from: String, to: String, token: String): F[Unit] = {
